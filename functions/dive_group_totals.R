dive_group_totals <- function(df) {
  # Define the column groups and the label column within the function
  diveGas <- c('# Air Dives', '# MIxed Gas Dives', '# Nitrox Dives')
  diveMode <- c('# Hookah Dives', '# Open Circuit Dives', '# Rebreather Dives', '# Surface Supplied Dives')
  divePlan <- c('# Computer Dives', '# Software Dives', '# Table Dives')
  diveDepth <- c('# 0-30ft Dives', '#31-60ft Dives', '#61-100ft Dives', '# 101-130ft Dives', '# 131-150ft Dives', '# 151-190ft Dives', '# 191+ft Dives')
  divePurpose <- c('# Science Dives', '#T/P Dives')
  diveEnviron <- c('# Aquarium Dives', '# Bluewater Dives', '# IcePolar Dives', '# Overhead Dives')
  diveDecomp <- c('# ReqDecomp Dives', '# Sat Dives')
  label_col <- 'OM'
  
  # Check if all columns in the groups and the label column exist in the DataFrame
  all_columns <- c(diveGas, diveMode, divePlan, diveDepth, divePurpose, diveEnviron, diveDecomp, label_col)
  if (!all(all_columns %in% colnames(df))) {
    stop("One or more columns do not exist in the DataFrame")
  }
